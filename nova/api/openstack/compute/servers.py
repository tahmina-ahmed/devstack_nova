...
class ServersTemplateAll(xmlutil.TemplateBuilder):
    def construct(self):
        root = xmlutil.TemplateElement('servers')
        elem = xmlutil.SubTemplateElement(root, 'server', selector='servers')
        make_server(elem, detailed=True)
...
class Controller(wsgi.Controller):
    ...
    def detail(self, req):
	    ...
	    print("server side detail req",req)
        except exception.Invalid as err:
            raise exc.HTTPBadRequest(explanation=err.format_message())
        return servers
    @wsgi.serializers(xml=ServersTemplateAll)
    def detail_all(self, req):
        """Returns a list of server details for a given user along with deleted ones."""
        try:
            servers = self._get_servers_all(req, is_detail=True)
	    print("server side detail_all req",req)
        except exception.Invalid as err:
            raise exc.HTTPBadRequest(explanation=err.format_message())
        return servers
    ...
    def _get_servers(self, req, is_detail):
        ...
        return response
    def _get_servers_all(self, req, is_detail):
        """Returns a list of servers, based on any search options specified."""
